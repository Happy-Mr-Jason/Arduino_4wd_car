...
public class MainActivity extends AppCompatActivity implements ContextListernerInterface {
    ...
    private Button[] cameraButtons = new Button[6];
    private int[] camerabuttonIDs = {R.id.btnTiltingNeutral, R.id.btnTiltingUp, R.id.btnTiltingDown, R.id.btnTiltingLeft, R.id.btnTiltingRight, R.id.btnTracking};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < cameraButtons.length; i++) {
            cameraButtons[i] = findViewById(camerabuttonIDs[i]);
        }
        ...
        for (int i = 0; i < cameraButtons.length; i++) {
            cameraButtons[i].setOnClickListener(myButtonClickListener);
        }
    }
...
//                        connectToSelectedDevice(items[which].toString());
    ...
    void sendData(String cmd, int data1, int data2) {
        ...
        Log.d("Send BT Data", String.valueOf(sendBytes));
        try {
